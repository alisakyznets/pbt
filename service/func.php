...
function fnGetLatestApplications () {
	...
	$applications_sql = "SELECT * FROM `applications` WHERE `status_id`='1' ORDER BY `created_at` DESC LIMIT 4";
	// выполнение запроса
...
function fnGetAdminApplications($status) {
	...
	$new_sql = "SELECT * FROM `applications` WHERE `status_id`='3' ORDER BY `created_at` DESC";
	// Запрос на получение всех заявок со статутом "Решена"
	$resolved_sql = "SELECT * FROM `applications` WHERE `status_id`='1' ORDER BY `created_at` DESC";
	// Запрос на получение всех заявок со статутом "Отклонена"
	$rejected_sql = "SELECT * FROM `applications` WHERE `status_id`='2' ORDER BY `created_at` DESC";
